...
pub async fn execute(tool_path: String, verbose: bool) -> Result<()> {
    ...
    // Check if WASM file exists - convert hyphens to underscores for Cargo's naming convention
    let wasm_filename = format!("{}.wasm", tool_name.replace('-', "_"));
    let wasm_path = tool_dir.join("target/wasm32-wasip1")
        .join(profile_dir)
        .join(&wasm_filename);
    ...
    if opt_wasm_path.exists() {
        ...
        println!("   Optimized WASM: {} ({}% reduction)", 
                ...
                if let Ok(history) = std::fs::read_to_string(&size_history_path) {
                    if let Ok(history_data) = serde_json::from_str::<serde_json::Value>(&history) {
                        if let Some(entries) = history_data.as_array() {
                            let current_size_in_history = entries.last()
    ...
    if wasm_size > 5_000_000 {
        suggestions.push("⚠️  Binary is larger than 5MB - optimization strongly recommended".to_string());
    }
    
    // Check optimization flags
    let has_opt_flags = manifest.optimization.flags.iter().any(|f| 
        f.contains("-O") || f.contains("--optimize")
    );
    ...
    match manifest.build.profile.as_str() {
        ...
        "release" => {
            if wasm_size > 1_000_000 {
                suggestions.push("Consider 'tiny' profile for maximum size reduction:\n     [build]\n     profile = \"tiny\"".to_string());
            }
        }
        _ => {}
    }
    ...
    suggestions.push("Review Cargo.toml for optimization opportunities:\n     - Remove unused dependencies\n     - Use default-features = false where possible\n     - Consider lighter alternatives to heavy crates".to_string());
        ...
        .exec()
    {
        ...
        if let Some(member_id) = workspace_member {
            if let Some(package) = metadata.packages.iter().find(|p| &p.id == member_id) {
                ...
                let significant_deps: Vec<_> = dep_weights.iter()
                    .take(5)
...
fn extract_size_from_section(section: &str) -> Option<u64> {
    ...
    if let Some(size_pos) = section.find("size ") {
        ...
        let size_end = size_str.find(|c: char| !c.is_numeric()).unwrap_or(size_str.len());
...
fn count_dependency_weight(metadata: &cargo_metadata::Metadata, dep_name: &str) -> usize {
    ...
    metadata.packages.iter()
        .filter(|p| p.name.contains(dep_name) || 
                   p.dependencies.iter().any(|d| d.name == dep_name))
        .count()
}
