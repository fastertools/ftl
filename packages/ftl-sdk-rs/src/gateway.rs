use crate::{
    mcp::{
        CallToolRequest, ErrorCode, InitializeRequest, InitializeResponse,
        ListToolsRequest, ListToolsResponse, McpProtocolVersion, ServerInfo,
    },
...
impl McpGateway {
    ...
    fn handle_initialize(&self, request: JsonRpcRequest) -> JsonRpcResponse {
        let params: InitializeRequest = match serde_json::from_value(request.params.unwrap_or(serde_json::json!({}))) {
            Ok(p) => p,
            Err(e) => {
    ...
    async fn handle_list_tools(&mut self, request: JsonRpcRequest) -> JsonRpcResponse {
        let _params: ListToolsRequest = match serde_json::from_value(request.params.unwrap_or(serde_json::json!({}))) {
            Ok(p) => p,
            Err(e) => {
                return JsonRpcResponse::error(
                    request.id,
        ...
        for endpoint in &self.config.tools.clone() {
            ...
            let tool_url = format!("http://{}.spin.internal/mcp", component_id);
            ...
            match self.forward_request(&tool_url, list_request).await {
                Ok(response) => {
                    if let Some(result) = response.result {
                        if let Ok(list_response) = serde_json::from_value::<ListToolsResponse>(result) {
    ...
    async fn handle_call_tool(&self, request: JsonRpcRequest) -> JsonRpcResponse {
        let params: CallToolRequest = match serde_json::from_value(request.params.unwrap_or(serde_json::json!({}))) {
            Ok(p) => p,
            Err(e) => {
                return JsonRpcResponse::error(
                    request.id,
...
macro_rules! ftl_mcp_gateway {
    ($config:expr) => {
        ...
        async fn handle_request(req: spin_sdk::http::Request) -> spin_sdk::http::Response {
            let handler = GATEWAY_HANDLER.get_or_init(|| {
                $crate::gateway::GatewayHandler::new($config)
            });
