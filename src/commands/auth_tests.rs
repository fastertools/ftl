...
impl CredentialsProvider for MockCredentialsProvider {
    fn get_stored_credentials(&self) -> Result<StoredCredentials, anyhow::Error> {
        if self.should_fail {
            Err(anyhow::anyhow!(
                self.error_message.clone().unwrap_or_else(|| "Failed to get credentials".to_string())
            ))
        } else {
...
async fn test_auth_status_logged_in_with_valid_token() {
    ...
    assert!(output.iter().any(|s| s.contains("AuthKit Domain: auth.example.com")));
    assert!(output.iter().any(|s| s.contains("Access Token: Valid for")));
    assert!(output.iter().any(|s| s.contains("Refresh Token: Available")));
}
...
async fn test_auth_status_logged_in_with_expired_token() {
    ...
    fixture.credentials_provider = Arc::new(
        MockCredentialsProvider::new().with_credentials(StoredCredentials {
            access_token: "test_token".to_string(),
            refresh_token: Some("refresh_token".to_string()),
            id_token: None,
    ...
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("✅ Logged in")));
    assert!(output.iter().any(|s| s.contains("Access Token: ⚠️  Expired")));
}
...
async fn test_auth_status_not_logged_in() {
    let mut fixture = TestFixture::new();
    fixture.credentials_provider = Arc::new(
        MockCredentialsProvider::new().with_failure("No matching entry found in keyring")
    );
    ...
    assert!(output.iter().any(|s| s.contains("Run ftl login to authenticate")));
}
...
async fn test_auth_status_error_checking_credentials() {
    let mut fixture = TestFixture::new();
    fixture.credentials_provider = Arc::new(
        MockCredentialsProvider::new().with_failure("Database connection failed")
    );
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("⚠️  Error checking authentication status")));
    assert!(output.iter().any(|s| s.contains("Run ftl login to re-authenticate")));
}
...
async fn test_auth_status_token_expiring_soon() {
    ...
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Access Token: Valid for 0h 45m")));
}
...
async fn test_auth_status_token_expiring_days_away() {
    ...
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Access Token: Valid for 171h 30m")));
}
...
async fn test_auth_status_various_error_messages() {
    ...
    for error_msg in not_found_errors {
        let mut fixture = TestFixture::new();
        fixture.credentials_provider = Arc::new(
            MockCredentialsProvider::new().with_failure(error_msg)
        );
