...
use crate::deps::{FtlApiClient, MessageStyle, UserInterface};
...
pub async fn list_with_deps(
    ...
    deps.ui.print_styled("Fetching applications...", MessageStyle::Cyan);
    
    let response = deps.api_client.list_apps().await?;
    
    if response.apps.is_empty() {
        deps.ui.print_styled("No applications found.", MessageStyle::Yellow);
        return Ok(());
    }
...
pub async fn status_with_deps(
    ...
    deps.ui.print_styled(
        &format!("Fetching status for '{app_name}'..."),
        MessageStyle::Cyan,
    );
...
pub async fn delete_with_deps(
    ...
    deps.ui.print_styled("Application to be deleted:", MessageStyle::Yellow);
    ...
    if !force && deps.ui.is_interactive() {
        ...
        if input != app_name {
            deps.ui.print_styled("Deletion cancelled.", MessageStyle::Yellow);
            return Ok(());
        }
    }
    
    deps.ui.print_styled("Deleting application...", MessageStyle::Cyan);
    ...
    if !response.warning.is_empty() {
        deps.ui.print("");
        deps.ui.print_styled(&response.warning, MessageStyle::Warning);
    }
...
fn display_apps_table(apps: &[types::ListAppsResponseAppsItem], deps: &Arc<AppDependencies>) {
    ...
    deps.ui.print(&format!("Total: {count} application{plural}"));
}

fn display_apps_json(apps: &[types::ListAppsResponseAppsItem], deps: &Arc<AppDependencies>) -> Result<()> {
    let app_infos: Vec<AppInfo> = apps.iter().map(|app| AppInfo {
        name: app.name.clone(),
        url: app.url.clone(),
    }).collect();
...
fn display_app_status_table(app: &types::GetAppStatusResponseApp, deps: &Arc<AppDependencies>) {
    ...
    deps.ui.print("");
    deps.ui.print_styled("Application Details", MessageStyle::Bold);
    let name = &app.name;
    deps.ui.print(&format!("  Name:         {name}"));
    let id = &app.id;
    ...
    let status_text = app.status.as_ref().map_or_else(|| "unknown".to_string(), std::string::ToString::to_string);
    ...
    if let Some(last_info) = &app.last_deployment_info {
        ...
        if let Some(error) = &last_info.error {
            deps.ui.print_styled(&format!("  Error:        {error}"), MessageStyle::Red);
        }
    }
...
fn display_app_status_json(app: &types::GetAppStatusResponseApp, deps: &Arc<AppDependencies>) -> Result<()> {
