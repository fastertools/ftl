...
async fn test_update_already_on_latest() {
    ...
    assert!(output.iter().any(|s| s.contains("Already on latest version")));
}
...
async fn test_update_new_version_available() {
    ...
    fixture.command_executor = Arc::new(
        MockCommandExecutor::new().expect_execute(
            "cargo",
    ...
    assert!(output.iter().any(|s| s.contains("Latest version available: 0.2.0")));
    assert!(output.iter().any(|s| s.contains("Installing latest version")));
    assert!(output.iter().any(|s| s.contains("FTL CLI updated successfully")));
}
...
async fn test_update_version_check_fails() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Could not check for latest version")));
    assert!(output.iter().any(|s| s.contains("Installing latest version")));
}
...
async fn test_update_install_fails() {
    ...
    assert!(result.unwrap_err().to_string().contains("Failed to update FTL CLI"));
}
...
async fn test_update_invalid_version_response() {
    ...
    fixture.http_client = Arc::new(
        MockHttpClient::new().with_response(r#"{"invalid": "json"}"#.to_string())
    );
...
async fn test_update_newer_current_version() {
    ...
    fixture.http_client = Arc::new(
        MockHttpClient::new().with_response(r#"{"crate": {"newest_version": "0.2.0"}}"#.to_string())
    );
...
async fn test_update_output_completeness() {
    ...
    for pattern in &expected_patterns {
        assert!(output.iter().any(|s| s.contains(pattern)), 
            "Expected to find '{}' in output", pattern);
    }
}