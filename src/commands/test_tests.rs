...
use crate::commands::test::{
    TestDependencies, execute_with_deps, DirectoryReader, FileChecker, TestCommandExecutor
};
...
impl MockTestCommandExecutor {
    ...
    fn expect_execute(mut self, command: &str, args: &[&str], cwd: Option<&str>, output: Output) -> Self {
    ...
    fn execute(
        ...
        *count += 1;
        ...
        if working_dir != expected_cwd.as_deref() {
            return Err(anyhow::anyhow!("Expected cwd {:?}, got {:?}", expected_cwd, working_dir));
        }
...
async fn test_single_rust_tool() {
    ...
    fixture.file_checker = Arc::new(
        MockFileChecker::new()
            .with_existing_file("./Cargo.toml")
    );
    
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("cargo", &["test"], Some("."), MockTestCommandExecutor::successful_output(
...
async fn test_single_npm_tool() {
    ...
    fixture.file_checker = Arc::new(
        MockFileChecker::new()
            .with_existing_file("./package.json")
    );
    
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("npm", &["test"], Some("."), MockTestCommandExecutor::successful_output(
...
async fn test_single_makefile_tool() {
    ...
    fixture.file_checker = Arc::new(
        MockFileChecker::new()
            .with_existing_file("./Makefile")
    );
    
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("make", &["test"], Some("."), MockTestCommandExecutor::successful_output(
                "All tests passing"
            ))
    );
...
async fn test_project_directory_multiple_tools() {
    ...
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("cargo", &["test"], Some("tool1"), MockTestCommandExecutor::successful_output(
                "test result: ok"
            ))
            .expect_execute("npm", &["test"], Some("tool2"), MockTestCommandExecutor::successful_output(
                "Tests passed"
            ))
    );
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Testing all tools in project")));
...
async fn test_no_test_configuration() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("No test configuration found")));
}
...
async fn test_cargo_test_failure() {
    ...
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("cargo", &["test"], Some("."), MockTestCommandExecutor::failed_output(
                "error: test failed"
            ))
    );
...
async fn test_custom_path() {
    ...
    fixture.file_checker = Arc::new(
        MockFileChecker::new()
            .with_existing_file("custom/path/Cargo.toml")
    );
    
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("cargo", &["test"], Some("custom/path"), 
...
async fn test_mixed_success_and_failure() {
    ...
    fixture.command_executor = Arc::new(
        MockTestCommandExecutor::new()
            .expect_execute("cargo", &["test"], Some("tool1"), 
                MockTestCommandExecutor::successful_output("test result: ok")
            )
            .expect_execute("npm", &["test"], Some("tool2"), 
