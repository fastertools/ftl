...
use crate::commands::setup::{
    SetupDependencies, templates_with_deps, info_with_deps,
    SpinInstaller, SetupCommandExecutor, Environment
};
...
async fn test_templates_already_installed() {
    ...
    assert!(output.iter().any(|s| s.contains("ftl-mcp templates are already installed")));
    assert!(output.iter().any(|s| s.contains("Use --force to reinstall")));
}
...
async fn test_templates_install_default() {
    ...
    fixture.command_executor = Arc::new(
        MockSetupCommandExecutor::new()
            ...
            .expect_execute(
                "/usr/local/bin/spin",
                &["templates", "install", "--git", "https://github.com/fastertools/ftl-mcp", "--upgrade"],
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Installing ftl-mcp templates from https://github.com/fastertools/ftl-mcp")));
    assert!(output.iter().any(|s| s.contains("Templates installed successfully!")));
...
async fn test_templates_install_from_git() {
    ...
    fixture.command_executor = Arc::new(
        MockSetupCommandExecutor::new()
            ...
            .expect_execute(
                "/usr/local/bin/spin",
                &["templates", "install", "--git", "https://github.com/user/repo", "--upgrade"],
                MockSetupCommandExecutor::successful_output("Templates installed"),
            )
            .expect_execute(
                "/usr/local/bin/spin",
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Installing templates from Git: https://github.com/user/repo")));
}
...
async fn test_templates_install_from_git_with_branch() {
    ...
    fixture.command_executor = Arc::new(
        MockSetupCommandExecutor::new()
            ...
            .expect_execute(
                "/usr/local/bin/spin",
                &["templates", "install", "--git", "https://github.com/user/repo", "--branch", "dev", "--upgrade"],
                MockSetupCommandExecutor::successful_output("Templates installed"),
            )
            .expect_execute(
                "/usr/local/bin/spin",
...
async fn test_templates_install_from_dir() {
    ...
    fixture.command_executor = Arc::new(
        MockSetupCommandExecutor::new()
            ...
            .expect_execute(
                "/usr/local/bin/spin",
                &["templates", "install", "--dir", "/path/to/templates", "--upgrade"],
                MockSetupCommandExecutor::successful_output("Templates installed"),
            )
            .expect_execute(
                "/usr/local/bin/spin",
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Installing templates from directory: /path/to/templates")));
}
...
async fn test_templates_install_from_tar() {
    ...
    fixture.command_executor = Arc::new(
        MockSetupCommandExecutor::new()
            ...
            .expect_execute(
                "/usr/local/bin/spin",
                &["templates", "install", "--tar", "templates.tar.gz", "--upgrade"],
                MockSetupCommandExecutor::successful_output("Templates installed"),
            )
            .expect_execute(
                "/usr/local/bin/spin",
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Installing templates from tarball: templates.tar.gz")));
}
...
async fn test_templates_install_failure() {
    ...
    assert!(result.unwrap_err().to_string().contains("Failed to install templates"));
}
...
async fn test_info_all_installed() {
    ...
    assert!(output.iter().any(|s| s.contains("ftl-mcp Templates: ✓ Installed")));
...
async fn test_info_spin_not_installed() {
    ...
    assert!(output.iter().any(|s| s.contains("Run 'ftl setup templates' to install")));
}
...
async fn test_info_no_templates() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("ftl-mcp Templates: ✗ Not installed")));
    assert!(output.iter().any(|s| s.contains("cargo-component: ✗ Not installed")));
