...
use crate::commands::registry::{
    RegistryDependencies, list_with_deps, search_with_deps, info_with_deps
};
...
async fn test_list_default_registry() {
    ...
    assert!(output.iter().any(|s| s.contains("Listing components from ghcr.io")));
    assert!(output.iter().any(|s| s.contains("Registry listing not yet implemented")));
...
async fn test_list_custom_registry() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Listing components from docker.io")));
}
...
async fn test_search_default_registry() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Searching for 'my-component' in ghcr.io")));
    assert!(output.iter().any(|s| s.contains("Registry search not yet implemented")));
    assert!(output.iter().any(|s| s.contains("https://github.com/search?q=mcp+my-component")));
}
...
async fn test_search_custom_registry() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Searching for 'test-tool' in quay.io")));
}
...
async fn test_search_with_special_characters() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Searching for 'my-component@v2.0'")));
    assert!(output.iter().any(|s| s.contains("https://github.com/search?q=mcp+my-component@v2.0")));
}
...
async fn test_info_simple_component() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Getting info for component: my-component")));
    assert!(output.iter().any(|s| s.contains("Registry info not yet implemented")));
    assert!(output.iter().any(|s| s.contains("Component reference formats:")));
    assert!(output.iter().any(|s| s.contains("ghcr.io/username/component:version")));
}
...
async fn test_info_full_component_reference() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Getting info for component: ghcr.io/ftl/my-tool:v1.0.0")));
}
...
async fn test_info_docker_hub_reference() {
    ...
    // Verify output
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Getting info for component: docker.io/library/nginx:latest")));
    assert!(output.iter().any(|s| s.contains("docker.io/username/component:version")));
}
