...
async fn test_check_and_install_spin_found_without_plugin() {
    ...
    assert!(output.iter().any(|s| s.contains("Installing Akamai plugin for Spin")));
}
...
async fn test_check_and_install_spin_not_found() {
    ...
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Please install Spin from")));
    assert!(output.iter().any(|s| s.contains("https://github.com/fermyon/spin")));
}
...
async fn test_check_and_install_plugin_list_fails() {
    ...
    let result = installer.check_and_install().await;
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Failed to list Spin plugins"));
}
...
async fn test_check_and_install_plugin_install_fails() {
    ...
    let output = ui.get_output();
    assert!(output.iter().any(|s| s.contains("Failed to install Akamai plugin")));
