...
var initCmd = &cobra.Command{
	...
	Short: "Initialize a new spindl project",
	Long: `Initialize a new spindl project with a template.

The init command creates a new project directory with a spindl.yml configuration
...
func runInit(projectName, template string) error {
	...
	configPath := filepath.Join(projectName, "spindl.yml")
...
`
	...
	fmt.Printf("  2. %s\n", infoColor.Sprint("Edit spindl.yml to configure your application"))
	fmt.Printf("  3. %s\n", infoColor.Sprint("spindl synth"))
...
func generateMCPConfig(projectName string) string {
	return fmt.Sprintf(`# spindl configuration
