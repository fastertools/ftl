...
import (
	...
	"github.com/fastertools/ftl-cli/go/spindl/pkg/construct"
)
...
func runConstructList() error {
	...
	for _, c := range constructs {
		...
		if c.Status == "stable" || c.Status == "preview" {
			fmt.Printf("    %s\n", dimColor.Sprintf("Usage: spindl init my-app --template %s", c.Name))
		}
		fmt.Println()
	}
	
	printInfo("Learn more:")
	fmt.Printf("  %s\n", dimColor.Sprint("Run 'spindl construct show <name>' for detailed information"))
...
func runConstructAdd(constructName string) error {
	...
	if constructDef == nil {
		...
		printInfo("Run 'spindl construct list' to see available constructs")
	...
	fmt.Printf("  1. %s\n", infoColor.Sprint("Create a new project: spindl init my-app --template "+constructName))
...
func runConstructShow(constructName string) error {
	...
	if constructDef.Status == "stable" || constructDef.Status == "preview" {
		...
		fmt.Printf("  %s\n", "spindl init my-app --template "+constructName)
		fmt.Println()
	}
