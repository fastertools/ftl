...
import (
	...
	"github.com/fastertools/ftl-cli/go/spin-compose/internal/schema"
)
...
func (e *Engine) SynthesizeConfig(configData []byte, format string) ([]byte, error) {
	...
	// Build the synthesis schema
	synthSchema, err := e.buildSynthesisSchema()
	if err != nil {
		return nil, fmt.Errorf("failed to build synthesis schema: %w", err)
	}
	
	// Unify input with schema
	unified := synthSchema.FillPath(cue.ParsePath("input"), inputValue)
	if err := unified.Validate(); err != nil {
		return nil, fmt.Errorf("configuration validation failed: %w", err)
	}
	
	// Extract the synthesized manifest
	manifestPath := cue.ParsePath("app.manifest")
	manifestValue := unified.LookupPath(manifestPath)
	if !manifestValue.Exists() {
		return nil, fmt.Errorf("failed to synthesize manifest")
	}
	...
	tomlData, err := e.marshalToTOML(manifest)
	if err != nil {
