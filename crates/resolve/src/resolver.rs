...
pub fn resolve_registry_component(
    ...
    let output = Command::new("wkg")
        .args(["oci", "pull", &registry_url, "-o", output_path.to_str().unwrap()])
...
fn parse_registry_ref(registry_ref: &str) -> Result<(String, String)> {
    ...
    let version_sep = registry_ref.rfind(':')
...
pub fn resolve_all_components(
    ...
    if !output_dir.exists() {
        std::fs::create_dir_all(&output_dir)
            .context("Failed to create .ftl/wasm directory")?;
    }
    ...
    if is_registry_ref(&ftl_resolve.mcp.authorizer) {
        components_to_resolve.push(("mcp-authorizer".to_string(), ftl_resolve.mcp.authorizer.clone()));
    }
    ...
    if !components_to_resolve.is_empty() {
        ...
        if no_cache {
            if components_to_resolve.len() == 1 {
                ...
                eprintln!("Downloading {} components in parallel (ignoring cache)...", components_to_resolve.len());
            }
        } else if all_cached {
            if components_to_resolve.len() == 1 {
            ...
            if components_to_resolve.len() == 1 {
                ...
                eprintln!("Resolving {} components in parallel...", components_to_resolve.len());
            }
        }
    }
    ...
    components_to_resolve.par_iter().for_each(|(name, registry_ref)| {
        ...
        match resolve_registry_component(registry_ref, &output_dir, no_cache) {
            ...
            Err(e) => {
                ...
                errors.lock().unwrap().push(format!("Failed to resolve {}: {}", name, e));
            }
        }
    });
