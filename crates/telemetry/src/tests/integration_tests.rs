...
#[cfg(test)]
mod tests {
    use crate::{TelemetryClient, config::TelemetryConfig, events::*};
    ...
    async fn test_telemetry_config_default() {
        ...
        assert_eq!(
            loaded_telemetry.installation_id,
            telemetry_config.installation_id
        );
    }
    ...
    async fn test_telemetry_client_with_custom_config() {
        ...
        // Log an event
        let event = TelemetryEvent::command_executed("test", vec![], "test-session".to_string());
    ...
    fn test_event_types() {
        ...
        assert_eq!(
            success.properties.get("duration_ms").unwrap(),
            &serde_json::json!(1234)
        );

        // Test command error event
        let error =
            TelemetryEvent::command_error("build", "compilation failed", "session-2".to_string());
        assert_eq!(error.event_type, EventType::CommandError);
        assert_eq!(
            error.properties.get("error").unwrap(),
    ...
    async fn test_telemetry_end_to_end_flow() {
        ...
        let client =
            TelemetryClient::from_config(telemetry_config).expect("Failed to create client");
        ...
        client
            ...
            .await
            .expect("Failed to log start event");

        // Log command success
        let success_event = TelemetryEvent::command_success("test", 1500, session_id.clone());
        client
    ...
    async fn test_telemetry_disabled_by_config() {
        ...
        let event = TelemetryEvent::command_executed("test", vec![], "session-123".to_string());
    ...
    async fn test_telemetry_error_event() {
        ...
        client
            ...
            .await
            .expect("Failed to log error event");
