...
fn test_get_cache_dir_with_xdg() {
    ...
    fixture.environment =
        Arc::new(MockEnvironment::new().with_var("XDG_CACHE_HOME", "/custom/cache"));
...
async fn test_fetch_latest_version_success() {
    ...
    fixture.http_client = Arc::new(
        MockHttpClient::new()
            .with_response(r#"{"crate": {"newest_version": "0.3.0"}}"#.to_string()),
    );
...
async fn test_fetch_latest_version_invalid_json() {
    let mut fixture = TestFixture::new();
    fixture.http_client = Arc::new(MockHttpClient::new().with_response("invalid json".to_string()));
...
async fn test_check_and_prompt_user_chooses_update() {
    ...
    assert!(
        output
            .iter()
            .any(|s| s.contains("A new version of FTL CLI is available"))
    );
}
...
fn test_save_version_cache_write_failure() {
    ...
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains("Failed to write version cache")
    );
}
