...
use toml_edit::{Item, Table, ArrayOfTables, InlineTable, Value, Array};
...
pub fn transpile_ftl_to_spin(ftl_config: &FtlConfig) -> Result<String> {
    ...
    vars_table["auth_gateway_url"] = create_default_var("http://ftl-mcp-gateway.spin.internal/mcp-internal");
    ...
    if let Some(oidc) = &ftl_config.auth.oidc {
        ...
        vars_table["auth_provider_authorize_endpoint"] = create_default_var(&oidc.authorize_endpoint);
    ...
    http_triggers.push(create_http_trigger("/.well-known/oauth-protected-resource", "mcp", false));
    http_triggers.push(create_http_trigger("/.well-known/oauth-authorization-server", "mcp", false));
...
fn create_mcp_component(version: &str) -> Table {
    ...
    vars["auth_provider_authorize_endpoint"] = toml_edit::value("{{ auth_provider_authorize_endpoint }}");
    vars["auth_provider_token_endpoint"] = toml_edit::value("{{ auth_provider_token_endpoint }}");
    vars["auth_provider_userinfo_endpoint"] = toml_edit::value("{{ auth_provider_userinfo_endpoint }}");
...
pub fn ensure_spin_toml(
    ...
    if file_system.exists(&ftl_toml_path) {
        let ftl_content = file_system.read_to_string(&ftl_toml_path)
        ...
        file_system.write_string(&spin_toml_path, &spin_content)
...
pub fn generate_temp_spin_toml(
    ...
    std::fs::write(&temp_file, &spin_content)
