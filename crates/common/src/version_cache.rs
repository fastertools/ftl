...
pub async fn check_and_prompt_for_update() -> Result<()> {
    ...
    let deps = Arc::new(VersionCacheDependencies {
        file_system: Arc::new(ftl_runtime::deps::RealFileSystem),
...
impl VersionCache {
    /// Create a new version cache instance
...
pub struct VersionCacheDependencies {
    /// File system for reading/writing cache files
    pub file_system: Arc<dyn FileSystem>,
    /// HTTP client for fetching version information
    pub http_client: Arc<dyn HttpClient>,
    /// Environment for system information
    pub environment: Arc<dyn Environment>,
    /// User interface for prompts and messages
    pub ui: Arc<dyn UserInterface>,
    /// Update executor for running update commands
...
impl VersionCacheManager {
    /// Create a new version cache manager with the given dependencies
