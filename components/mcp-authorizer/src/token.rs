...
pub async fn verify(token: &str, provider: &Provider, store: &Store) -> Result<TokenInfo> {
    ...
    let decoding_key = if let Some(public_key) = &provider.public_key {
        ...
        return Err(AuthError::Configuration("No key source configured".to_string()));
    };
    ...
    let algorithm = match provider.algorithm.as_deref().unwrap_or("RS256") {
        ...
        alg => return Err(AuthError::Configuration(format!("Unsupported algorithm: {}", alg))),
    };
...
fn extract_scopes(claims: &Claims) -> Vec<String> {
    ...
    if let Some(scp) = &claims.scp {
        return match scp {
            ScopeValue::String(s) => s.split_whitespace()
                .map(String::from)
                .collect(),
