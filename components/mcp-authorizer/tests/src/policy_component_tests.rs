...
"#;
    ...
    request
        .set_path_with_query(Some("/mcp/x/data-processor"))
        .unwrap();
    ...
    assert_eq!(
        response.status(),
        200,
        "Should allow data-processor component"
    );
    ...
    request.set_method(&http::types::Method::Get).unwrap();
    request
        .set_path_with_query(Some("/mcp/x/other-component"))
        .unwrap();
...
fn test_component_with_subpath() {
    ...
    request
        .set_path_with_query(Some("/mcp/x/api-gateway/readonly"))
        .unwrap();
    ...
    assert_eq!(
        response.status(),
        200,
        "Should extract component from path with subpath"
    );
}
...
"#;
    ...
    assert_eq!(
        response.status(),
        401,
        "Should deny when no component in path"
    );
    ...
    assert_eq!(
        response.status(),
        401,
        "Should deny root path (no component)"
    );
}
...
"#;
    ...
    request.set_method(&http::types::Method::Get).unwrap();
    request
        .set_path_with_query(Some("/mcp/x/frontend"))
        .unwrap();
    ...
    let headers = http::types::Headers::new();
    headers
        .append("authorization", format!("Bearer {}", bob_token).as_bytes())
        .unwrap();
    let request = http::types::OutgoingRequest::new(headers);
    request.set_method(&http::types::Method::Get).unwrap();
    request
...
"#;
    ...
    request.set_method(&http::types::Method::Get).unwrap();
    request
        .set_path_with_query(Some("/mcp/x/admin-panel"))
        .unwrap();
    ...
    let headers = http::types::Headers::new();
    headers
        .append("authorization", format!("Bearer {}", anon_token).as_bytes())
        .unwrap();
    let request = http::types::OutgoingRequest::new(headers);
    request.set_method(&http::types::Method::Get).unwrap();
    request
        .set_path_with_query(Some("/mcp/x/public-api"))
        .unwrap();
