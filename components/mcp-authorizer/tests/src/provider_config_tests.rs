...
fn test_https_enforcement_all_urls() {
    ...
    variables::set(
        "mcp_oauth_authorize_endpoint",
        "http://insecure.example.com/auth",
    ); // HTTP should fail
    variables::set(
        "mcp_oauth_token_endpoint",
        "https://secure.example.com/token",
    );
...
/// Create a JWT token
fn create_token(private_key: &RsaPrivateKey, issuer: &str, audience: Option<&str>) -> String {
    let now = Utc::now();
    let claims = Claims {
