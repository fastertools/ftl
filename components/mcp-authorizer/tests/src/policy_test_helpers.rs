...
/// Generate a test keypair for policy tests
...
pub fn setup_subject_check_policy(allowed_subjects: Vec<&str>) {
    let subjects_list = allowed_subjects
        .iter()
        .map(|s| format!("\"{}\"", s))
        .collect::<Vec<_>>()
        .join(", ");
...
pub fn setup_component_policy(allowed_components: Vec<&str>) -> (RsaPrivateKey, RsaPublicKey) {
    ...
    let components_list = allowed_components
        .iter()
        .map(|c| format!("\"{}\"", c))
        .collect::<Vec<_>>()
        .join(", ");
...
/// Returns the keypair for use in token creation
pub fn setup_tool_authorization_policy(
    allowed_tools: Vec<&str>,
    dangerous_tools: Vec<&str>,
    ...
    let dangerous_list = dangerous_tools
        .iter()
        .map(|t| format!("\"{}\"", t))
        .collect::<Vec<_>>()
        .join(", ");
...
pub fn create_policy_test_token_with_key(
    ...
    let claims = Claims {
        ...
        roles: if roles.is_empty() {
            ...
            Some(roles.iter().map(|s| s.to_string()).collect())
        },
        additional,
    };
