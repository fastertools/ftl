...
use spin_test_sdk::{
    bindings::{
        fermyon::spin_test_virt::variables, fermyon::spin_wasi_virt::http_handler,
        wasi::http::types,
    },
    spin_test,
};
...
fn test_authkit_jwks_auto_derivation() {
    ...
    assert_eq!(
        metadata["jwks_uri"],
        "https://test-project.authkit.app/oauth2/jwks"
    );
    assert_eq!(
        metadata["authorization_endpoint"],
        "https://test-project.authkit.app/oauth2/authorize"
    );
    assert_eq!(
        metadata["token_endpoint"],
        "https://test-project.authkit.app/oauth2/token"
    );
    assert_eq!(
        metadata["registration_endpoint"],
        "https://test-project.authkit.app/oauth2/register"
    );
}
...
fn test_authkit_openid_configuration() {
    ...
    let request = types::OutgoingRequest::new(types::Headers::new());
    request
        .set_path_with_query(Some("/.well-known/openid-configuration"))
        .unwrap();
    let response = spin_test_sdk::perform_request(request);
