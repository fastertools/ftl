use crate::{
    jwt_verification_tests::{configure_test_provider, create_test_token, AudienceValue, Claims},
    ResponseData,
};
use spin_test_sdk::{
...
fn test_gateway_response_passthrough() {
    ...
    crate::jwt_verification_tests::mock_jwks_endpoint(
        "https://test.authkit.app/.well-known/jwks.json",
        jwks,
    );
    ...
    let gateway_header = response_data
        .find_header("x-gateway-response")
        .map(|v| String::from_utf8_lossy(v));
    assert_eq!(
        gateway_header.as_deref(),
        Some("success"),
    ...
    assert!(
        response_data
            .find_header("access-control-allow-origin")
            .is_some(),
...
fn test_gateway_error_passthrough() {
    ...
    headers.append("content-type", b"application/json").unwrap();
    headers
        .append("x-gateway-error", b"internal-failure")
        .unwrap();
    ...
    let token = create_test_token(
        &private_key,
        crate::jwt_verification_tests::Claims {
            sub: "user123".to_string(),
            iss: "https://test.authkit.app".to_string(),
            aud: Some(AudienceValue::Single("test-audience".to_string())),
    ...
    let gateway_error_header = response_data
        .find_header("x-gateway-error")
        .map(|v| String::from_utf8_lossy(v));
...
fn test_various_token_scenarios() {
    ...
    let token_no_client_id = create_test_token(
        &private_key,
        crate::jwt_verification_tests::Claims {
    ...
    headers
        .append(
            "authorization",
            format!("Bearer {}", token_no_client_id).as_bytes(),
        )
        .unwrap();
    let request = http::types::OutgoingRequest::new(headers);
    request.set_path_with_query(Some("/mcp")).unwrap();
    ...
    assert_eq!(
        response.status(),
        200,
        "Token without client_id should succeed using sub as fallback"
    );

    // Test 2: Token with explicit client_id different from sub
    let token_with_client_id = create_test_token(
        &private_key,
        crate::jwt_verification_tests::Claims {
            sub: "user123".to_string(),
            iss: "https://test.authkit.app".to_string(),
    ...
    headers2
        .append(
            "authorization",
            format!("Bearer {}", token_with_client_id).as_bytes(),
        )
        .unwrap();
    ...
    assert_eq!(
        response2.status(),
        200,
        "Token with explicit client_id should succeed"
    );
}
...
fn test_auth_failure_error_format() {
    ...
    let headers = http::types::Headers::new();
    headers
        .append("authorization", b"Bearer invalid.jwt.token")
        .unwrap();
    let request = http::types::OutgoingRequest::new(headers);
    request.set_path_with_query(Some("/mcp")).unwrap();
    ...
    assert!(
        ...
        "Error response must have 'error_description' field"
    );

    // Verify WWW-Authenticate header
    let www_auth = response_data
        .find_header("www-authenticate")
    ...
    assert!(
        auth_str.contains("error="),
        "WWW-Authenticate must contain error parameter"
    );
}
