...
fn test_policy_allow_all() {
    ...
    assert_eq!(response.status(), 200, "Allow-all policy should permit access");
}
...
fn test_policy_deny_all() {
    ...
    // Should deny access
    assert_eq!(response.status(), 401, "Deny-all policy should block access");
    
    // Check error message
    let body = response.body().unwrap();
    let body_str = String::from_utf8_lossy(&body);
    assert!(body_str.contains("Access denied by authorization policy"), 
...
"#;
    ...
    let body = response.body().unwrap();
    let body_str = String::from_utf8_lossy(&body);
    assert!(body_str.contains("server_error"), "Should indicate server error");
}
