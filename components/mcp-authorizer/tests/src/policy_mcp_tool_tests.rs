...
fn test_mcp_tool_call_authorization() {
    ...
    assert_eq!(
        response.status(),
        401,
        "Should deny dangerous tool without admin"
    );
...
"#;
    ...
    assert_eq!(
        response.status(),
        401,
        "Should deny when MCP context not available"
    );
    ...
    assert_eq!(
        response.status(),
        401,
        "Should deny GET request (no MCP context)"
    );
}
...
"#;
    ...
    let reader_token = create_policy_test_token_with_key(
        &private_key,
        "reader",
        vec![],
        vec![("scp", serde_json::json!(["user:read"]))],
    );
    ...
    headers
        .append(
            "authorization",
            format!("Bearer {}", reader_token).as_bytes(),
        )
        .unwrap();
    headers.append("content-type", b"application/json").unwrap();
    let request = http::types::OutgoingRequest::new(headers);
    ...
    assert_eq!(
        response.status(),
        401,
        "Should deny create without write scope"
    );
}
