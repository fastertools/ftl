...
use crate::jwt_verification_tests::{AudienceValue, Claims};
...
fn test_jwks_caching() {
    ...
    headers1
        .append("authorization", format!("Bearer {}", token1).as_bytes())
        .unwrap();
    headers1
    ...
    assert_eq!(
        call_count_after_first, 1,
        "JWKS should be fetched on first request"
    );
    ...
    assert!(
        cached_jwks.is_some(),
        "JWKS should be in cache after first request"
    );
    ...
    headers2
        .append("authorization", format!("Bearer {}", token2).as_bytes())
        .unwrap();
    headers2
        .append("content-type", b"application/json")
        .unwrap();
    ...
    assert_eq!(
        call_count_after_second, 1,
        "JWKS should be cached and not fetched again"
    );
...
fn test_jwks_cache_ttl() {
    ...
    assert!(
        cached_json.get("jwks").is_some(),
        "Cached data should contain jwks"
    );
    assert!(
        cached_json.get("expires_at").is_some(),
        "Cached data should contain expiry"
    );
}
...
fn test_jwks_cache_per_issuer() {
    ...
    variables::set(
        "mcp_jwt_jwks_uri",
        "https://issuer1.com/.well-known/jwks.json",
    );
    variables::set("mcp_jwt_audience", "test-audience");
