...
fn test_scoped_tool_call_unprefixed() {
    ...
    assert!(response_json["result"]["content"][0]["text"].as_str().unwrap().contains("Result: 5"));
}
...
fn test_individual_tool_scope_rejected() {
    ...
    let request_json = create_json_rpc_request("tools/list", None, Some(serde_json::json!(1)));
    let request = create_mcp_request(request_json);
    request.set_path_with_query(Some("/mcp/x/calc/add")).unwrap();
    ...
    let response_json = response_data.body_json().expect("Expected JSON response");
    assert!(response_json["error"].as_str().unwrap().contains("Invalid path"));
}
...
fn test_invalid_path_outside_mcp() {
    ...
    assert_eq!(response_data.status, 404);
    let response_json = response_data.body_json().expect("Expected JSON response");
    assert!(response_json["error"].as_str().unwrap().contains("must start with /mcp"));
}
...
fn test_readonly_header_blocks_execution() {
    ...
    let response_json = response_data.body_json().expect("Expected JSON response");
    assert!(response_json["error"].is_object());
    assert!(response_json["error"]["message"].as_str().unwrap().contains("readonly mode"));
}
...
fn test_toolsets_header_filters_components() {
    ...
    assert!(tools.len() >= 1, "Should have at least calc component tools");
    
    // Verify the tools we get are from calc (prefixed since we're at /mcp)
    for tool in tools {
        let name = tool["name"].as_str().unwrap();
        assert!(name.starts_with("calc__"), "Tool {} should be from calc component", name);
    }
}
...
fn test_toolsets_header_multiple_components() {
    ...
    for tool in tools {
        let name = tool["name"].as_str().unwrap();
        assert!(name.starts_with("calc__"), "Should only have calc tools when filtered");
    }
}
